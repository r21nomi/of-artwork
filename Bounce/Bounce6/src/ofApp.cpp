...
void ofApp::initStairs() {
    ...
    for (int i = 0; i <= count; i++) {
        ...
        float y = i * height;
        stairs.push_back(new Stair(i + 1, ofVec2f(0, y), ofGetWidth(), ofGetHeight() - y, ofColor(stairColor, stairAlpha)));
    }
}
...
//    ofBeginShape();
//    ofVertex(skewLeft, 0);
//    ofVertex(ofGetWidth() - skewRight, 0);
...
Ball* ofApp::getBall() {
    ...
    float x = ofRandom(skewLeft, ofGetWidth() - skewRight);
...
void ofApp::mouseReleased(int x, int y, int button){
    Stair* stair = new Stair(stairs.size() + 1, ofVec2f(0, y), ofGetWidth(), ofGetHeight() - y, ofColor(stairColor, 255));
    
    // Insert to particular position.
    vector<Stair*>::iterator it = stairs.begin();
    while (it != stairs.end()) {
        Stair *item = dynamic_cast<Stair*>(*it);
        Stair *nextItem = dynamic_cast<Stair*>(*it + 1);
        
        if (nextItem != NULL && item->getStairY() > y && nextItem->getStairY() < y) {
            it = stairs.insert(it, stair);
            break;
        } else {
            ++it;
        }
    ...
    int step = 1;
    float alpha = 255 / stairs.size();
    for (Stair* s : stairs) {
        float stairAlpha = ofMap(alpha * step, alpha, 255, 160, 255);
        s->update(step, ofColor(stairColor, stairAlpha));
        step++;
    }
}
