...
void ofApp::update(){
    ...
    for (int i = 0; i < delaunay.triangleMesh.getNumIndices() / 3; i++) {
        ...
        ITRIANGLE it = delaunay.getTriangleAtIndex(idx1);
        
        mesh.addVertex(v1);
        mesh.addVertex(v2);
        mesh.addVertex(v3);
        
        ofColor color1 = vertexPoints[idx1]->getColor();
        ofColor color2 = vertexPoints[idx2]->getColor();
        ofColor color3 = vertexPoints[idx3]->getColor();
        
        // FIXME: (color1 + color2 + color3) / 3 will be dark color.
        ofColor color = (color1 + color2) / 1.35;
