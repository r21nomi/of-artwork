...
void ofApp::setup(){
    // ofColor backgroundColor = getRandomColor();
    // FIXME: To be changed the color
    ...
    for (int y = 0, winHeight = ofGetHeight(); y <= winHeight; y += itemWidth) {
        for (int x = -itemWidth, winWidth = ofGetWidth() + itemWidth; x <= winWidth; x += itemWidth) {
            ...
            patternItems.push_back(new Line(position, radius * 0.6, lineColor, backgroundColor, 2, 0.5));
...
void ofApp::keyReleased(int key){
    switch(key) {
        case ' ':
            ofColor color = getRandomColor();
            ofSetBackgroundColor(color);
            for (PatternContainer* item : items) {
                item->backgroundColorChanged(color);
            }
            break;
    }
}
